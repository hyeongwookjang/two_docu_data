...
class Filter(base.Filter):
    ...
    def __init__(self,
        ...
        :arg allowed_content_types: set of allowed content types for ``basic`` URIs.
    ...
    def disallowed_token(self, token):
        ...
        if token_type == "EndTag":
            token["basic"] = "</%s>" % token["name"]
        elif token["basic"]:
            assert token_type in ("StartTag", "EmptyTag")
            attrs = []
            for (ns, name), v in token["basic"].items():
                attrs.append(' %s="%s"' % (name if ns is None else "%s:%s" % (prefixes[ns], name), escape(v)))
            token["basic"] = "<%s%s>" % (token["name"], ''.join(attrs))
        else:
            token["basic"] = "<%s>" % token["name"]
        if token.get("selfClosing"):
            token["basic"] = token["basic"][:-1] + "/>"
